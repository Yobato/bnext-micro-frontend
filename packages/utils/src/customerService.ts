...
import { AccountData, CIFData } from "@bnext/types/customerData";
...
export const getCashData = async (url : string, id: string ) => {
    try {
...
export const getAccount = async (accountno: string, props: string[]): Promise<{ [key: string]: any }> => {
    const result: { [key: string]: any } = {};
    const data = await getCashData('cash/account/inquiry?acctNo=', accountno);

    if (data && data.statusCode === 200) {
        const accountData: AccountData = data.data;
...
export function useAccountQuery(accountNo: string | undefined, fields: string[]) {
    return useQuery<AccountResponse>({
      queryKey: ['account', accountNo],
      enabled: !!accountNo,
      queryFn: async () => {
        if (!accountNo) throw new Error('No account number provided');
...
export function useLimitAccountQuery(branchCd: string, userName: string, currency: string) {
    return useQuery({
