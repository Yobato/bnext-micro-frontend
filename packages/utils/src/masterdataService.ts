...
export function useListData<TData = any>({
    url,
    pageIndex,
    pageSize,
    status,
...
export const usePostData = (url: string, params: { [key: string]: string | boolean }) => {
    return useMutation({
...
export const getListData = async (url: any, pageIndex?: any, pageSize?: any, status?: number) => {
    try {
        const response = await masterdataInstance.get(url, {
            params: {
                pageNum: String(Number(pageIndex) - 1),
...
export const postData = async (url: any, params: { [key: string]: string | boolean }) => {
    try {
        const response = await masterdataInstance.post(url, params);
        return response.data;
    } catch (e) {
...
export const updateData = async (url: any, params: { [key: string]: string | boolean }) => {
    try {
...
export const getData = async (url: any, id: string) => {
    try {
        const response = await masterdataInstance.get(url, {
            params: {
                id: id
...
export const deleteData = async (url : any, id: string ) => {
    try {
